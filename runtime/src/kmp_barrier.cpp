                            ...
                            USE_ITT_BUILD_ARG(void * itt_sync_obj) )
{
    ...
    if (!KMP_MASTER_TID(tid)) {
        ...
        ANNOTATE_NEW_BARRIER_BEGIN(this_thr);
        ...
        for (i=1; i<nproc; ++i) {
            ...
            ANNOTATE_NEW_BARRIER_END(other_threads[i]);
#if USE_ITT_BUILD && USE_ITT_NOTIFY
    ...
    if (KMP_MASTER_TID(tid)) {
        ...
        if (nproc > 1) {
            ...
            for (i=1; i<nproc; ++i) {
                ...
                ANNOTATE_NEW_BARRIER_BEGIN(other_threads[i]);
        ...
        flag.wait(this_thr, TRUE
                  USE_ITT_BUILD_ARG(itt_sync_obj) );
        ANNOTATE_NEW_BARRIER_END(this_thr);
#if USE_ITT_BUILD && USE_ITT_NOTIFY
                          ...
                          USE_ITT_BUILD_ARG(void *itt_sync_obj) )
{
    ...
    if (child_tid < nproc) {
        ...
        do {
            ...
            flag.wait(this_thr, FALSE
                      USE_ITT_BUILD_ARG(itt_sync_obj) );
            ANNOTATE_NEW_BARRIER_END(child_thr);
#if USE_ITT_BUILD && USE_ITT_NOTIFY
                           ...
                           USE_ITT_BUILD_ARG(void *itt_sync_obj) )
{
    ...
    if (child_tid < nproc) {
        ...
        do {
            ...
            ANNOTATE_NEW_BARRIER_BEGIN(child_thr);
                ...
                if (reduce) {
                    ...
                    for (child_tid=tid+1; child_tid<=tid+thr_bar->leaf_kids; ++child_tid) {
                        KA_TRACE(100, ("__kmp_hierarchical_barrier_gather: T#%d(%d:%d) += T#%d(%d:%d)\n",
                                       gtid, team->t.t_id, tid, __kmp_gtid_from_tid(child_tid, team),
                                       team->t.t_id, child_tid));
                        ANNOTATE_NEW_BARRIER_END(other_threads[child_tid]);
